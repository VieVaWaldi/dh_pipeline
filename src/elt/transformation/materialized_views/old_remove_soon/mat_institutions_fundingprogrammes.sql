...
CREATE MATERIALIZED VIEW core_mats.mat_institutions_fundingprogrammes AS
SELECT 
...
INNER JOIN core.j_project_fundingprogramme as pfp ON pfp.project_id = p.id
INNER JOIN core.fundingprogramme as f ON pfp.fundingprogramme_id = f.id
GROUP BY i.id;
...
SELECT count(*) FROM core_mats.mat_institutions_fundingprogrammes
LIMIT 100;
...
SELECT pg_size_pretty(pg_total_relation_size('core_mats.mat_institutions_fundingprogrammes'));

-- Data size without indexes
SELECT pg_size_pretty(pg_relation_size('core_mats.mat_institutions_fundingprogrammes'));
